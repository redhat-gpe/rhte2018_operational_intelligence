...
=== GiTHub URL and Justification of the usage of Notebooks in Today's Data Analysis Prod World

The below link is the GitHub URL for this code. Main.scala is the program is applied on the Zeppelin Notebook.
Since we have used SparkSQL, we can keep on querying the data with different results to get different statistics. This is how the NoteBook Environments like Jupyter or Zeppelin is drawn attention and used popularly in most of the production deployments. We can interactive visualize the results and change the query plan according to our convenience, accuracy level and perfection. 

https://github.com/gpe-mw-training/operational_intelligence/tree/master/uber-data-analysis

Zeppelin has a support for Multiple Interpretors like Scala, Python, Kotlin, HBase (DataBase), It is OpenSource and other visualization tools like Tableau, R, QlikSense (which used for statistical analysis) bears licensing cost.

Zeppelin has a built in Scala and Python Intepretor, it is as good as you type in Scala shell or Python Shell gives the exact same result. But with zeppelin you will get the visualization benefits and notebook advantage.

For your understanding, Below here an example for scala shell. A simple word count application and it can be run on Zeppelin NoteBook as well.

----

[root@localhost bin]# spark-shell
2018-08-27 14:38:03 WARN  Utils:66 - Your hostname, localhost.localdomain resolves to a loopback address: 127.0.0.1; using 192.168.0.2 instead (on interface wlp58s0)
2018-08-27 14:38:03 WARN  Utils:66 - Set SPARK_LOCAL_IP if you need to bind to another address
2018-08-27 14:38:03 WARN  SparkConf:66 - The configuration key 'spark.executor.port' has been deprecated as of Spark 2.0.0 and may be removed in the future. Not used any more
2018-08-27 14:38:04 WARN  NativeCodeLoader:62 - Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
Setting default log level to "WARN".
To adjust logging level use sc.setLogLevel(newLevel). For SparkR, use setLogLevel(newLevel).
2018-08-27 14:38:04 WARN  SparkConf:66 - The configuration key 'spark.executor.port' has been deprecated as of Spark 2.0.0 and may be removed in the future. Not used any more
2018-08-27 14:38:09 WARN  SparkConf:66 - The configuration key 'spark.executor.port' has been deprecated as of Spark 2.0.0 and may be removed in the future. Not used any more
2018-08-27 14:38:09 WARN  SparkConf:66 - The configuration key 'spark.executor.port' has been deprecated as of Spark 2.0.0 and may be removed in the future. Not used any more
Spark context Web UI available at http://192.168.0.2:4040
Spark context available as 'sc' (master = local[*], app id = local-1535360890347).
...
scala> wc.saveAsTextFile("README.count1")
...
. Output can be viewed here which is equivalent to write the code on Zeppelin or Juypter Notebook.

----
root@localhost bin]# ls
beeline          derby.log             find-spark-home.cmd  load-spark-env.sh  pyspark2.cmd  README.count1  run-example.cmd   spark-class.cmd  sparkR.cmd        spark-shell.cmd  spark-sql.cmd      spark-submit.cmd
beeline.cmd      docker-image-tool.sh  input                metastore_db       pyspark.cmd   Readme.md      spark-class       sparkR           spark-shell       spark-sql        spark-submit
com.redhat.gpte  find-spark-home       load-spark-env.cmd   pyspark            Readme.count  run-example    spark-class2.cmd  sparkR2.cmd      spark-shell2.cmd  spark-sql2.cmd   spark-submit2.cmd
[root@localhost bin]# cd README.count1/
[root@localhost README.count1]# ls
part-00000  part-00001  _SUCCESS
[root@localhost README.count1]# ls -lt README.count1
ls: cannot access 'README.count1': No such file or directory
[root@localhost README.count1]# view part-00000
[root@localhost README.count1]# 

----
. Output of the Main.scala program ran from IDE, is uploaded in our GitHub.

https://github.com/gpe-mw-training/operational_intelligence/blob/master/output/output.log
