...
. In this section we will review the Kafka Connect deployment. Clone the Kafka distribution to the client VM. 
+
-----
$ git clone https://github.com/gpe-mw-training/rhte2018.git
-----

. Navigate to the strimzi directory. In kafka-connect.yaml, under the spec object, review the configuration
+
----
  config:
...
. Navigate to where you have cloned the lab assets. Under ../operational_intelligence/uber-data-analysis/src/main/resources/data, review the input data file (uber.csv). Copy this file to the /tmp directory. The contents of this directory will be streamed by Kafka connect to the Kafka broker.

. A topic (my-topic-2) that Kafka Connect uses to publish to the Kafka broker has been created for you

. Create a topic consumer for this topic in a manner similar to how you created the additional consumer in the earlier section. Create a deployment yaml (connector-consumer.yaml) in the strimzi/ directory for this consumer and make sure that the top of the env section looks like
+
-----
env:
          - name: BOOTSTRAP_SERVERS
